...
Stmt* Parser::parseVariableDeclaration() {
	...
	VariableDeclaration* declaration = new VariableDeclaration(type, identifier, this->parseExpr(), isConst);	// MEMORY LEAK
...
Expr* Parser::parseAssignmentExpr() {
	...
	if (this->tokens[0].getType() == TokenType::Equals) {
		this->eat();
		return new AssignmentExpr(left, this->parseAssignmentExpr());
	}
	return left;
}
...
Expr* Parser::parseExponentionalExpr() {
	...
	while (this->tokens[0].getValue() == "^")
		left = new BinaryExpr(eat().getValue(), left, parsePrimaryExpr());
	return left;
}
...
Expr* Parser::parseForExpr() {
    ...
    std::unordered_map<std::string, ForVariables> variables;
    this->eat();
...
Expr* Parser::parseWhileExpr() {
    ...
    std::cout << this->tokens[0].getType();
...
Expr* Parser::parsePrimaryExpr() {
    std::cout << "asdf";
    std::cout << (this->tokens[0].getType() == TokenType::While) << std::endl;
	...
	case TokenType::Number:
		return new NumericLiteral(std::stod(this->eat().getValue())); 	// MEMORY LEAK
	case TokenType::Character:
	case TokenType::Characters:
		return new CharactersLiteral(this->eat().getValue(), tk == TokenType::Characters); 	// MEMORY LEAK
	...
	default: {
		std::cout << std::endl;
        std::cout << (this->tokens[0].getType() == TokenType::While);
...
Program* Parser::produceAST(std::string sourceCode) {
	...
	Program* program = new Program();	// MEMORY LEAK
